...
const outputFolder = path.join(__dirname, "nextInterval");
...
// Function to find the closest trade within 5 seconds later
function findClosestTrade(trades, targetTimestamp) {
  return trades
    .filter(trade => trade.timestamp > targetTimestamp && trade.timestamp <= targetTimestamp + 5)
...
fs.readFile(buysOnPumpFile, "utf8", (err, data) => {
  ...
  buysOnPump.forEach(buyTrade => {
    ...
    fs.readFile(tradeFile, "utf8", (err, tradeData) => {
      ...
      // Find the closest trade within 5 seconds later
      const closestTrade = findClosestTrade(trades, matchingTrade.timestamp);

      if (!closestTrade) {
        console.warn(`No trade found within 5 seconds later for ${matchingTrade.signature}`);
        return;
      }
